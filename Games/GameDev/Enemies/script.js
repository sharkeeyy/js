...
class Enemy3 {
    ...
    update() {
        this.x = this.curve * Math.sin(this.angle * Math.PI / 180) - this.width / 2 + CANVAS_WIDTH / 2;
        this.y = this.curve * Math.cos(this.angle * Math.PI / 180) - this.height / 2 + CANVAS_HEIGHT / 2;
        this.angle += this.angleSpeed;
        if (this.x + this.width < 0) this.x = CANVAS_WIDTH;
        if (gameFrame % this.flapSpeed === 0) {
            this.frame > 4 ? this.frame = 0 : this.frame++;
